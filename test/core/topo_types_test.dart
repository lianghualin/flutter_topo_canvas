...
void main() {
  group('TopoNode', () {
    test('constructs with id and data, position optional', () {
      const node = TopoNode<String>(id: 'n1', data: 'hello');
      expect(node.id, 'n1');
    ...
    test('position stores Offset when provided', () {
      const node = TopoNode<int>(id: 'n1', data: 42, position: Offset(10, 20));
  ...
  group('TopoEdge', () {
    test('constructs with from/to node ids', () {
      const edge = TopoEdge<String>(id: 'e1', fromNodeId: 'a', toNodeId: 'b', data: 'link');
      expect(edge.id, 'e1');
  ...
  group('TopoGroup', () {
    ...
    test('isAbnormal defaults to false', () {
      const group = TopoGroup(id: 'g1', label: 'd', nodeIds: ['n1']);
